...
def rabin_karp(pattern, text):
    ...
    p_sum = plus_minus(pattern_ascii) # hash function (pattern)
    ...
    for i in range(len(text_ascii)-n + 1): # for each text fragment (pattern length)
        ...
        if p_sum == plus_minus(text_ascii[i:i+n]): # if hash(pattern) equals hash(fragment)
